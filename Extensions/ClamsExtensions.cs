...
namespace server.Extensions
{
    public static class ClamsExtensions
    {
        ...
        {
            return user.Claims.SingleOrDefault(x => x.Type.Equals("FirstName"))?.Value ?? string.Empty;
        }

        public static string GetLastName(this ClaimsPrincipal user)
        {
            return user.Claims.SingleOrDefault(x => x.Type.Equals("LastName"))?.Value ?? string.Empty;
        }
    }
}
